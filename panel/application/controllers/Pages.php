...
class Pages extends MY_Controller
{
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            foreach($_FILES["img_url"]["name"] as $key => $value):
                if ($value == "") :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Sayfa Eklenirken Hata Oluştu. Sayfa Görseli Seçtiğinizden Emin Olup, Lütfen Tekrar Deneyin."]);
                    die();
                endif;
    ...
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            foreach(json_decode($page->img_url,true) as $key => $value):
