...
class Testimonials extends MY_Controller
{
    ...
    public function datatable()
    {
        ...
        foreach ($items as $item) {
            ...
            foreach($content as $key => $value){
                $content[$key] = mb_word_wrap($value,30,"...");
            }
            $item->content = json_encode($content,JSON_UNESCAPED_UNICODE);
            ...
            $data[] = array($item->rank, '<i class="fa fa-arrows" data-id="' . $item->id . '"></i>', $item->id, $item->title, $item->full_name, $item->content, $checkbox, turkishDate("d F Y, l H:i:s", $item->createdAt), turkishDate("d F Y, l H:i:s", $item->updatedAt), json_encode($sharedAt,JSON_UNESCAPED_UNICODE), $proccessing);
        }
    ...
    public function update($id)
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            foreach(json_decode($testimonial->img_url,true) as $key => $value):
    ...
    public function delete($id)
    {
        ...
        if (!empty($testimonial)) :
            ...
            if ($testimonial_model) :
				$url = FCPATH . "uploads/testimonials_v/{$testimonial->img_url}";
